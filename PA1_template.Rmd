...
```
The peak activity interval returned in the `peakInterval` variable is **`r peakInterval`**.
...
```
Per the `countNA` variable we have **`r countNA`** missing values in the `activity` data frame.  
<br>
...
```{r WeekdaysVsWeekendsActivityImputed}
